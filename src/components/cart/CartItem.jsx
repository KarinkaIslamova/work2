...
const CartItem = (props) => {
  ...
  const onAddOverlay=async(obj)=>{
    try{
      const findOverlay=props.overleyItems.find(objOver=>objOver.myId===obj.myId)
      if(findOverlay){
        ...
        props.setOverlayItems([...props.overleyItems,data])
      }
    }
    catch{
  ...
  const onAddFav=async(obj)=>{
    try{
      ...
      if(findFavorites){
        ...
        props.setFavorites([...props.overlayFavorites,data])
      }
    }
    catch{
      alert("Произошла ошибка")
